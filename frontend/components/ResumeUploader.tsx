...
export default function ResumeUploader({ onParsed }: { onParsed: (resume: ResumeRecord, matches: JobMatch[]) => void }) {
  ...
  async function handleFile(file: File) {
    ...
    if (file.size > MAX_FILE_SIZE) {
      setError("Please choose a resume smaller than 8 MB.");
      return;
    }
  ...
  return (
    <div>
      ...
      >
        ...
        <p className="mt-2 text-sm text-slate-500">{uploading ? "Parsing profile and finding relevant opportunities" : "or click to browse · PDF or DOCX · up to 8 MB"}</p>
