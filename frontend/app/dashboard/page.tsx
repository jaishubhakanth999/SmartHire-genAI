    ...
    <main className="min-h-[calc(100vh-64px)] bg-slate-50/70">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-6 sm:py-10">
        <div className="rounded-3xl bg-slate-950 p-7 text-white shadow-2xl shadow-indigo-100 sm:p-10">
          ...
          <div className="mt-7 grid gap-3 sm:grid-cols-3">{["1. Upload resume", "2. Understand profile", "3. Match + improve"].map((item) => <div key={item} className="rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-xs font-semibold text-slate-200">{item}</div>)}</div>
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[300px_1fr]">
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7"><ResumeUploader onParsed={(r, m) => { setResume(r); setMatches(m); setCvHistory([]); setHistory((prev) => [r, ...prev.filter((x) => x.id !== r.id)]); }} /></section>
        </div>
